...
function Conversation({ studentId, teacherId }) {
  const [messages, setMessages] = useState([]);
  const [newMessage, setNewMessage] = useState("");

  useEffect(() => {
    fetchMessages();
  }, [teacherId]); // Fetch messages whenever teacherId changes

  const fetchMessages = async () => {
    try {
      const response = await axios.get(`/getstudentchats/${teacherId}`);
  ...
  const sendMessage = async () => {
    try {
      await axios.post(`/studentchats/${teacherId}`, { message: newMessage });
      setNewMessage("");
